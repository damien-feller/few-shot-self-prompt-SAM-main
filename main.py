...
def visualize_predictions(org_img, images, masks, model, num_samples=3, val=False, eval_num=0):
    ...
    for i in range(num_samples):
        ...
        combo_heat = cv2.normalize(combo_heat, None, 0, 255, cv2.NORM_MINMAX)
        combo_gaussian = heatmap_normalized + (1 * edges_gaussian)
        combo_gaussian = cv2.normalize(combo_gaussian, None, 0, 255, cv2.NORM_MINMAX)
        combo_median = heatmap_normalized + (1 * edges_median)
        combo_median = cv2.normalize(combo_median, None, 0, 255, cv2.NORM_MINMAX)
